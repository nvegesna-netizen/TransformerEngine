...
def test_checkpoint_eval_forward_train_backward_pairs_recompute_stash(use_reentrant):
    ...
    The first phase must decide whether a stash is needed from autograd participation, not
    just module mode. The phase-2 restore then follows the recorded stash count rather than
    re-reading a mode that may have changed. The always-training module is an in-test control.
    """
    FP8GlobalStateManager.reset()
    fp8_recipe = recipe.DelayedScaling(fp8_format=recipe.Format.HYBRID)
